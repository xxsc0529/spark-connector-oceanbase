...
import OceanBaseSparkDataSource.{
  writeDataViaDirectLoad,
...
class OceanBaseSparkDataSource extends JdbcRelationProvider {
  ...
  override def createRelation(
    ...
    new OceanBaseJDBCRelation(schema, parts, jdbcOptions)(sparkSession)
  }
  ...
  private def buildJDBCOptions(
    ...
    if (parameters.contains(JDBCOptions.JDBC_QUERY_STRING)) {
      paraMap = paraMap + (JDBCOptions.JDBC_QUERY_STRING -> parameters(
        JDBCOptions.JDBC_QUERY_STRING
      ))
    } else {
      paraMap =
        paraMap + (JDBCOptions.JDBC_TABLE_NAME -> getDbTable(oceanBaseConfig))
    }
...
object OceanBaseSparkDataSource {
  ...
  /** Writes DataFrame data using OceanBase's direct-load feature
    ...
    * @note
    *   Direct load is OceanBase's high-performance data loading feature that
    *   writes directly to storage layer
    */
    ...
    mode match {
      ...
      case _ =>
        throw new NotImplementedError(
          s"${mode.name()} mode is not currently supported."
        )
    }
